 function [nlev, pmid, pint, tmid, tint, n_to_cut, h2o, o3, n2o, co, ch4, o2, cflag, cldfrc_tmp] = read_profile_allsky(i,j,pmid_tmp,tmid_tmp,psfc,tsfc,h2o_raw,o3_raw,cloudfrc)
...
n2o = 0.323 * ones(1,length(h2o)) * 1e-6;
co  = 0.15  * ones(1,length(h2o)) * 1e-6;
ch4 = 1.797 * ones(1,length(h2o)) * 1e-6;
...
if (sum(cldfrc_tmp > 0))
  ...
  cflag.icld = 2;	%0-no cloudy%1-RANDOM overlap assmption(IMAC=0 or 1)%2-MAXIMUM/RANDUM(IMAC=0 or 1)% 3-MAXIMUM(IMAC=1)
else
  cflag.imca = 0;
  cflag.icld = 0;
