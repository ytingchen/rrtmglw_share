...
% check merra time manually
  outfile  = [outdir,'output_lw_era5emis_land_',time,'_allsky_2xco2.nc'];
  
  %%%add checkpoint for grid consistence!!
  % read data
...
% Create struct for output data
  ...
  cloudfrc = ncread([locdata,'ERA5/cloud_cover/hourly/',time,'.nc'],'cc',[1 1 1 tind],[Inf Inf Inf 1]);
  ciwc_raw = ncread([locdata,'ERA5/cloud_ice/hourly/',time,'.nc'],'ciwc',[1 1 1 tind],[Inf Inf Inf 1]);
  clwc_raw = ncread([locdata,'ERA5/cloud_liquid/hourly/',time,'.nc'],'clwc',[1 1 1 tind],[Inf Inf Inf 1]);
...
% for loop for each gridbox.
  ...
  for i = 1: 1: length(lon_coor)
    ...
    for j = 1: 1: length(lat_coor)
      ...
      if any(ismissing(h2o_raw(i,j,:))|ismissing(o3_raw(i,j,:))) % half part only for derived emis | any(ismissing(emis_tmp((i-1)/8+1,(j-1)/8+1)))
        ...
        [nlev, pmid, pint, tmid, tint, n_to_cut, h2o, o3, n2o, co, ch4, o2, cflag, cldfrc_tmp] = read_profile_allsky(i,j,pmid_tmp,tmid_tmp,psfc,tsfc,h2o_raw,o3_raw,cloudfrc);
        ...
        if cflag.imca == 1
          ...
          ciwc   = ciwc(n_to_cut+1:dim_output(3)-1);
          clwc   = clwc(n_to_cut+1:dim_output(3)-1);
          cld_writer(pint,nlev,cldfrc_tmp,ciwc,clwc);

        end
        % calculate longwave 
        [fup_lw, fdn_lw, fnt_lw, htr_lw] = rrtmg_lw_htr_allsky(cflag,atmprofile, nlev);
  ...
  clear tmid_tmp h2o_raw o3_raw psfc_raw tsfc_raw cloudfrc ciwc_raw clwc_raw psfc_land tsfc_land emis_tmp
